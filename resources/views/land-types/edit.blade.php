...
<div class="card card-theme">
    <div class="card-body">
        <form action="{{ route('land-types.update', $landType) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="mb-3">
                <label for="name" class="form-label">Name <span class="text-danger">*</span></label>
                <input type="text" class="form-control form-control-theme @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $landType->name) }}" required maxlength="255">
                @error('name')
            ...
            <div class="mb-3">
                ...
                <select name="party_sub_category_id" id="party_sub_category_id" class="form-select form-select-theme @error('party_sub_category_id') is-invalid @enderror">
                    ...
                    @foreach($partySubCategories as $sc)
                        <option value="{{ $sc->id }}" @selected(old('party_sub_category_id', $landType->party_sub_category_id) == $sc->id)>{{ $sc->category?->name }} — {{ $sc->name }}</option>
                    @endforeach
                </select>
